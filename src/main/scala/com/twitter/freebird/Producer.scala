...
sealed trait Producer[P <: Platform[P], T] {
  def map[U](fn: T => U): Producer[P, U] = MapProducer(this, fn)

  def flatMap[U](fn: T => TraversableOnce[U]): Producer[P, U] = FlatMapProducer(this, fn)

  def filter(fn: T => Boolean): Producer[P, U] = FilterProducer(this, fn)

  def group[K, V](implicit ev: T <:< (K, V)): KeyedProducer[P, K, V] =
    GroupProducer(this.asInstanceOf[Producer[P, (K, V)]])
...
sealed trait SortedKeyedProducer[P <: FreePlatform, K, V] extends KeyedProducer[P, K, V] {
  def fold[U](init: U)(fn: (U, V) => U): KeyedProducer[P, K, U] =
}

case class Source[P <: StreamPlatform[P], T](source: P#Source[T])
  extends Producer[P, T]

case class MapProducer[P <: StreamPlatform[P], T, U](parent: Producer[P, T], fn: T => U)
  extends Producer[P, U]

case class FlatMapProducer[P <: StreamPlatform[P], T, U](parent: Producer[P, T], fn: T => TraversableOnce[U])
  extends Producer[P, U]

case class FilterProducer[P <: StreamPlatform[P], T](parent: Producer[P, T], fn: T => Boolean)
  extends Producer[P, T]

case class GroupProducer[P <: StreamPlatform[P], K, V](parent: Producer[P, (K, V)])
...
case class SortedFold[P <: FreePlatform[P], K, V, U](parent: SortedKeyedProducer[P, K, V], init: U, fn: (U, V) => U)
  extends KeyedProducer[P, K, U]
